...
channels = [
    "ticker.BTC-29DEC23-16000-C.raw",
    "ticker.BTC-29DEC23-18000-C.raw",
...
class DeribitFetching:
    ...
    def connectws(self, deribit_url):
        logger.info(
            "Establishing connection to Deribit WebSocket {}".format(deribit_url)
        )
        try:
    ...
    async def runner(self) -> None:
        ...
        async with self.connectws(self.deribit_url) as self.ws_client:
            ...
            self.loop.create_task(self.reauthenticate())

            logger.info(
                "subscribing to receive data for the following channels {}",
                format(channels),
            )
                ...
                if "id" in list(msg):
                    if msg["id"] == 9929:
                        if self.refresh_token is None:
                            x = 1
                            logger.info(
                                "Authenticated with User {},".format(self.client_id)
                            )
                        else:
    ...
    async def subscription(self, channels: list) -> None:
        # Request to subscribe to the provided channels
        add_params: dict = {"channels": channels}

        self.params["method"] = "public/subscribe"
        self.params["id"] = 42
        self.params["params"] = add_params
        await self.ws_client.send(json.dumps(self.params))
    ...
    async def auth(self) -> None:
        ...
        self.params["params"] = add_params
        logger.info(json.dumps(self.params))
        ...
        while True:
            if self.expiration_time is not None:
                if datetime.utcnow() > self.expiration_time:
                    ...
                    self.params["params"] = add_params
                    logger.info(
                        "Re-authenticating using provided user {}".format(
                            self.client_id
                        )
                    )
                    await self.ws_client.send(json.dumps(self.params))
...
if __name__ == "__main__":
    ...
    logger.add(
        sys.stderr,
        format="{thread.name} | {time} | {level} | {message}",
        serialize=True,
        enqueue=True,
    )
    try:
        DeribitFetching(
            client_id=client_id, client_secret=client_secret, channels=channels
        )
